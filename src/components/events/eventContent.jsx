...
const EventContent = () => {
  ...
  const data = useSelector((state) => state.event.newData);
    ...
    <>
      <Stack direction={"row"}>
        <IconButton onClick={() => navigate("/events")}>
          <IconBack
            style={{ width: "2rem", height: "2rem", fill: "#2469d8" }}
          />
        </IconButton>
      </Stack>
      <Grid container direction={"row"} sx={style}>
          ...
          <Stack direction={"column"} p={2} spacing={2}>
            <Stack direction={"row"} spacing={4}>
              ...
              <Stack direction={"row"} spacing={2} alignItems={"center"}>
                ...
                <Typography variant="body2" color={"primary.light"}>
                  {moment(data.startDate, "YYYY-MM-DD").format("Do MMMM YYYY")}
                </Typography>
            ...
            <Stack direction={"row"} justifyContent={"space-between"}>
              <Typography variant="h5">{data.eventName}</Typography>
              <Button
                startIcon={
                  <IconStarActive
        ...
        <Grid item xs={4} pl={2}>
          <Paper sx={{ width: "100%", height: 500 }}></Paper>
        </Grid>
      </Grid>
    </>
  );
